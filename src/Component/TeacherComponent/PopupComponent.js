...
import { formStore } from '../../Store/TeacherStore/FormStore';
import { teacherStore } from '../../Store/TeacherStore/TeacherStore';
import { RiAddCircleLine, RiCheckboxCircleLine, RiErrorWarningLine } from 'react-icons/ri';
import axios from 'axios';
import InputMask from 'react-input-mask';
import '../../Component/TeacherComponent/PopupStyle.css';
import { validateTeacherForm } from '../../Screen/Teachers/FormTeacherValidator';
...
const PopupComponent = observer(({ onSubmit, teacherId }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (validateTeacherForm()) {
      ...
      if (teacherId) {
        await axios
          .put(`https://dummy.restapiexample.com/api/v1/update/${teacherId}`, payload)
          .then((response) => {
            console.log(response.data);
            onSubmit();
            teacherStore.setPopupOpen(false);
            formStore.setShowSuccessMessage(true);
            setTimeout(() => {
              formStore.setShowSuccessMessage(false);
            }, 2000);
          })
          .catch((error) => {
            console.error(error);
            formStore.setShowErrorMessage(true);
            setTimeout(() => {
              formStore.setShowErrorMessage(false);
            }, 2000);
          });
      } else {
        await axios
          ...
          .then((response) => {
            ...
            teacherStore.setPopupOpen(false);
            formStore.setShowSuccessMessage(true);
            console.log(formStore.showSuccessMessage)
            setTimeout(() => {
              formStore.setShowSuccessMessage(false);
            }, 5000);
            console.log(formStore.showSuccessMessage)
          ...
          .catch((error) => {
            ...
            teacherStore.setPopupOpen(false);
            formStore.setShowErrorMessage(true);
            console.log(formStore.showErrorMessage)
            setTimeout(() => {
              formStore.setShowErrorMessage(false);
            }, 5000);
            console.log(formStore.showErrorMessage)
    ...
    <div className="popup-container">
      <div className="popup-content">
        ...
        <form onSubmit={handleFormSubmit} className="teacher-popup-form">
          ...
          {toJS(formStore.showSuccessMessage)&& (
           ...
           {toJS(formStore.showErrorMessage) && (
